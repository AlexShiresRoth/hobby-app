<script lang="ts">
	...
	const handleSaveProfile: SubmitFunction = () => {
		...
		return async ({ update, result }) => {
			...
			if (result.type === 'success') {
				const data = result.data as { hobbyId: string };

				goto(`/dashboard/hobby/${data.hobbyId}`);
			}
		};
	};
		...
		{#if mounted}
			{#if !started}
				<h1 class="text-6xl font-bold">
					<p>Looking to try something</p>
					{#key current}
						<p in:fly={{ y: 20 }}>{ideas[current]}</p>
					{/key}
				</h1>
				<button
			...
			{:else}
				...
				{#if hobbySuggestion && !generating}
					<div class="flex w-1/2 flex-col gap-2">
						...
						<p>{hobbySuggestion.living_environment}</p>
